...
//
//  この選び方は後々まで効く。CloudKit は本番へ流した項目を取り消せない
//  (Production のスキーマは一方通行)
...
final class BookmarkSync {
    ...
    // 印 (isApplyingRemote) でも止めているが、あれは同じ流れの中でしか効かない。
    // 時間を跨いだ往復——例えば窓を手前に戻すたびに一度ずつ送り直す類——はこちらで止める
    ...
    private func apply(_ record: CKRecord) {
        ...
        // 中身が同じで差し替えを見送る場合でも控えるのが要点。
        // ここを通さないと、次に窓を手前に戻した時に同じ物をもう一度送り返してしまう。
        syncedPayload = data
