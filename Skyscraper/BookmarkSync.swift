...
final class BookmarkSync {
    ...
    // 降ってきた分を流し込んでいる最中の印。
    // BookmarkStore の didSet はこちらを叩くようにしてあるので、
    // これが無いと「受け取る→変わった→送り返す」で往復し続ける
    ...
    private func download() async {
        guard await isUsable, let store else { return }
        ...
        do {
            ...
            if !store.bookmarks.isEmpty { await push() }
        } catch {
            report("取り込み", error)
        }
    }
    ...
    private func apply(_ record: CKRecord) {
        ...
        guard incoming != store.bookmarks else { return }
        ...
        print("BookmarkSync: pulled \(incoming.count) bookmarks")
    }
    ...
    private func push() async {
        guard await isUsable, let store else { return }
        guard let data = try? JSONEncoder().encode(store.bookmarks) else { return }
        ...
        do {
            ...
            print("BookmarkSync: pushed \(store.bookmarks.count) bookmarks")
            ...
            do {
                ...
                print("BookmarkSync: pushed (overwrote server copy)")
            } catch {
    ...
    private func report(_ what: String, _ error: Error) {
        if let ckError = error as? CKError {
            print("BookmarkSync: \(what) failed — \(ckError.code.rawValue) \(ckError.localizedDescription)")
        } else {
            print("BookmarkSync: \(what) failed — \(error.localizedDescription)")
        }
    }
}
